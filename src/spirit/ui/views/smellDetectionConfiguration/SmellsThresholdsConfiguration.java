...
public class SmellsThresholdsConfiguration extends Dialog {
	...
	private LongMethodDetectionConfiguration longMethodConf;
	...
	public SmellsThresholdsConfiguration(Shell parentShell, SpIRITSmellsView spIRITView) {
		...
		longMethodConf = DataBaseManager.getInstance().getLongMethodDetectionConfiguration(projectName);
		...
		if (list.getSelectionCount() == 1) {
			...
			if (currentSelection.equals("Brain Class")) {
				...
				if (currentSelection.equals("Brain Method")) {
					...
					if (currentSelection.equals("Long Method")) {
						loadControlsForLongMethod();
					} else {
		...
		if (currentSelection != null) {
			if (currentSelection.equals("Brain Class")) {
				...
				if (currentSelection.equals("Brain Method")) {
					...
					if (currentSelection.equals("Long Method")) {
						longMethodConf.setMLOC_VERY_HIGH(new Double(currentTexts.get(0).getText()));
					} else {
						if (currentSelection.equals("Data Class")) {
							...
							if (currentSelection.equals("Dispersed Coupling")) {
								...
								dipersedCouplingConf
										.setCDISP_GreaterEqual_Half(new Double(currentTexts.get(1).getText()));
								dipersedCouplingConf
										.setMAXNESTING_Greater_Shallow(new Double(currentTexts.get(2).getText()));
							} else {
								if (currentSelection.equals("Feature Envy")) {
									...
									if (currentSelection.equals("God Class")) {
										...
										godClassConf.setWMC_GreaterEqual_VeryHigh(
												new Double(currentTexts.get(1).getText()));
										godClassConf.setTCC_Less_OneThird(new Double(currentTexts.get(2).getText()));
									} else {
										if (currentSelection.equals("Intensive Coupling")) {
											intensiveCouplingConf
													.setCINT_Greater_SMemCap(new Double(currentTexts.get(0).getText()));
											intensiveCouplingConf
													.setCDISP_Less_Half(new Double(currentTexts.get(1).getText()));
											intensiveCouplingConf
													.setCINT_Greater_Few(new Double(currentTexts.get(2).getText()));
											intensiveCouplingConf.setCDISP_Less_OneQuarter(
													new Double(currentTexts.get(3).getText()));
											intensiveCouplingConf.setMAXNESTING_Greater_SHALLOW(
													new Double(currentTexts.get(4).getText()));
										} else {
											if (currentSelection.equals("Refused Parent Bequest")) {
												refusedParentBequestConf.setNProtM_Greater_Few(
														new Double(currentTexts.get(0).getText()));
												refusedParentBequestConf.setBUR_Less_OneThird(
														new Double(currentTexts.get(1).getText()));
												refusedParentBequestConf.setBOvR_Less_OneThird(
														new Double(currentTexts.get(2).getText()));
												refusedParentBequestConf.setAMW_Greater_AMWAvg(
														new Double(currentTexts.get(3).getText()));
												refusedParentBequestConf.setWMC_Greater_WMCAvg(
														new Double(currentTexts.get(4).getText()));
												refusedParentBequestConf.setNOM_Greater_NOMAvg(
														new Double(currentTexts.get(5).getText()));
											} else {
												if (currentSelection.equals("Shotgun Surgery")) {
													shotgunSurgeryConf.setCM_Greater_SMemCap(
															new Double(currentTexts.get(0).getText()));
													shotgunSurgeryConf.setCC_GreaterEqual_MANY(
															new Double(currentTexts.get(1).getText()));
												} else {
													if (currentSelection.equals("Tradition Breaker")) {
														traditionBreakerConf.setNAS_GreaterEqual_NOMAvg(
																new Double(currentTexts.get(0).getText()));
														traditionBreakerConf.setPNAS_GreaterEqual_TWO_THIRD(
																new Double(currentTexts.get(1).getText()));
														traditionBreakerConf.setAMW_Greater_AMWAvg(
																new Double(currentTexts.get(2).getText()));
														traditionBreakerConf.setWMC_GreaterEqual_VeryHigh(
																new Double(currentTexts.get(3).getText()));
														traditionBreakerConf.setNOM_GreatherEqual_High(
																new Double(currentTexts.get(4).getText()));
														traditionBreakerConf.setAMW_Greater_AMWAvg_2(
																new Double(currentTexts.get(5).getText()));
														traditionBreakerConf.setNOM_GreatherEqual_HighDiv2(
																new Double(currentTexts.get(6).getText()));
														traditionBreakerConf.setWMC_Greater_VeryHighDiv2(
																new Double(currentTexts.get(7).getText()));
													}
												}
											}
										}
	...
	private void loadControlsForLongMethod() {
		createLabel(xFirstIndentation, 15, 347, "Method is excessively large");
		createLabel(xSecondIndentation, 35, 67, "LOC >");
		createText(35);

		currentTexts.get(0).setText(new Double(longMethodConf.getMLOC_VERY_HIGH()).toString());
	}
	...
	private void okButtonPressed() {
		...
		DataBaseManager.getInstance().saveOrUpdateEntity(longMethodConf);
		...
		RankingManagerForSmells.getInstance().setDesingFlaws(flaws,
				RankingManagerForSmells.getInstance().getCurrentProject());
