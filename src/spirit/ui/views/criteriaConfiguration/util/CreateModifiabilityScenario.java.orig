...
public class CreateModifiabilityScenario extends Dialog {
	...
	private void selectClassClicked() {
		IStructuredSelection selection = (IStructuredSelection) allClassesViewer
			      .getSelection();
		if (selection.size() > 0){
			addNewClassToScenario((String) selection.getFirstElement());
		}
	}
	
	protected void addNewClassToScenario(String className){
	...
	private void selectPackageClicked() {
		IStructuredSelection selection = (IStructuredSelection) allPackagesViewer
			      .getSelection();
		if (selection.size() > 0){
			addNewPackageToScenario((String) selection.getFirstElement());
		}
		
	}
	
	protected void addNewPackageToScenario(String packageName){
	...
	protected void okButtonClicked() {
		modifiabilityScenariosConfiguration.addScenario(new ModifiabilityScenario(getScenarioName(), getScenarioDescription(), selectedClasses, selectedPackages, getImportance(), currentProject.getFullPath().toString() ));
	}
	...
	protected void setScenarioName(String name){
		text.setText(name);
	}
	protected void setScenarioDescription(String name){
		text_1.setText(name);
	}
	protected void loadImportanceValue(Double value){
		comboViewer.getCombo().select(importanceValues.indexOf(value));
	}
	...
	protected Double getImportance(){
		return getComboSelection(comboViewer);
	}
}
