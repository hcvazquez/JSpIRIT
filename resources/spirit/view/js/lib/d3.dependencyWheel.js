...
d3.chart.dependencyWheel = function(options) {
  ...
  function chart(selection) {
    selection.each(function(data) {
      ...
      .append("g")
        .attr("transform", "translate(" + (width / 2 + 200) + "," + (width / 2 + 200) + ")");
      ...
      var fill = function(d) {
        ...
        //return "hsl(" + parseInt(((packageNames[d.index][0].charCodeAt() - 97) / 26) * 360, 10) + ",90%,70%)";
          if(colors[packageNames[d.index]] === undefined)
              return "hsl(0,100%,100%)";
          else
            //return "hsl(" + colors[packageNames[d.index]].toLowerCase() + ")";          
              return "hsl(0,100%," + colors[packageNames[d.index]] + "%)";
      };

      // Returns an event handler for fading a given chord group.
      var fade = function(opacity, showSmells) {
        return function(g, i) {
          svg.selectAll(".chord")
              .filter(function(d) {
          ...
          if (showSmells) {
            $scope.selectionName = packages[i].name;
            $scope.selectionSmells = packages[i].smells;                          
            $scope.$apply();
          }
          var groups = [];
          svg.selectAll(".chord")
